...
data:
  ...
  bundledCode: "#line 1 \"verify/yosupo/point_set_range_composite.test.cpp\"\n#define\
    ...
    \ update(int i) { dat[i] = M::op(dat[2 * i], dat[2 * i + 1]); }\n\npublic:\n \
    \ SegmentTree() {}\n  SegmentTree(int n) : SegmentTree(vc<S>(n, M::e())) {}\n\
    \  template <class Iter>\n  SegmentTree(const Iter &bg, const Iter &ed) : SegmentTree(vc<S>(bg,\
    \ ed)) {}\n  template <class T>\n  SegmentTree(const vc<T> &vec) : n(vec.size()),\
