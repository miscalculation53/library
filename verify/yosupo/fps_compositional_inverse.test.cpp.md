...
data:
  ...
  bundledCode: "#line 1 \"verify/yosupo/fps_compositional_inverse.test.cpp\"\n#define\
    \ PROBLEM \"https://judge.yosupo.jp/problem/compositional_inverse_of_formal_power_series_large\"\
    ...
    \ - T(l) >= T(k));\n  vc<T> res(k);\n  internal::random_sample_range<does_sort>(res,\
    \ T(l), T(r));\n  return res;\n}\n#line 2 \"math/fps/compositional_inverse.hpp\"\
    \n\n#line 4 \"math/fps/compositional_inverse.hpp\"\n\n#line 2 \"math/fps/fps.hpp\"\
    ...
    \ c); }\n};\n#line 2 \"math/fps/power_projection.hpp\"\n\n#line 4 \"math/fps/power_projection.hpp\"\
    ...
    \ >> (32 - countr_zero((uint)pw2));\n}\n#line 7 \"math/fps/power_projection.hpp\"\
    ...
    \ 1);\n    reverse(ALL(g2)), g2.resize(n2), reverse(ALL(g2));\n    return power_projection(n2\
    \ - 1, m, f, g2);\n  }\n  n++;\n  int k = 1;\n  if (ntt_ok<mint>(4 * n))\n  {\n\
    \    const mint iz = mint(4 * n).inv();\n    const mint iroot = info.iroot[bit_width(2\
    \ * n)];\n    vc<mint> ipw(2 * n);\n    {\n      mint tmp = 1;\n      repi(i,\
    \ 2 * n) ipw[bitrev(2 * n, i)] = tmp, tmp *= iroot;\n    }\n    F p = g, q = -f,\
    \ p2, q2;\n    p.resize(n), q.resize(n);\n    while (n > 1)\n    {\n      // x\
    \ \u306B\u3064\u3044\u3066\u9577\u3055 n \u3067\u3001y \u306B\u3064\u3044\u3066\
    \u9577\u3055 k\n      p2.assign(4 * n * k, 0), q2.assign(4 * n * k, 0);\n    \
    \  repi(j, k) repi(i, n)\n      {\n        p2[j * (2 * n) + i] = p[j * n + i];\n\
    \        q2[j * (2 * n) + i] = q[j * n + i];\n      }\n      ntt(p2), ntt(q2);\n\
    \      repi(i, 2 * n * k)\n      {\n        const mint x = q2[2 * i], y = q2[2\
    \ * i + 1];\n        p2[i] = (p2[2 * i] * y - p2[2 * i + 1] * x) * ipw[i];\n \
    \       q2[i] = 2 * x * y;\n      }\n      p2.resize(2 * n * k), q2.resize(2 *\
    \ n * k);\n      intt(p2), intt(q2);\n      repi(i, 2 * n * k) p2[i] *= iz, q2[i]\
    \ *= iz;\n      repi(j, k) repi(i, n / 2)\n      {\n        p2[(j + k) * n + i]\
    \ += p[j * n + (2 * i + 1)];\n        q2[(j + k) * n + i] += 2 * q[j * n + (2\
    \ * i)];\n      }\n      n >>= 1, k <<= 1;\n      repi(j, k) repi(i, n)\n    \
    \  {\n        p[j * n + i] = p2[j * (2 * n) + i];\n        q[j * n + i] = q2[j\
    \ * (2 * n) + i];\n      }\n    }\n    return (p.rev().pre(m) * (F{1} + (q.rev()\
    \ << 1)).inv(m)).pre(m);\n  }\n  else\n  {\n    F p = g, q = -f, p2(2 * n), q2(2\
    \ * n), r(2 * n);\n    p.resize(n), q.resize(n);\n    while (n > 1)\n    {\n \
    \     // x \u306B\u3064\u3044\u3066\u9577\u3055 n \u3067\u3001y \u306B\u3064\u3044\
    \u3066\u9577\u3055 k\n      p2.resize(2 * n * k), q2.resize(2 * n * k);\n    \
    \  fill(ALL(p2), 0), fill(ALL(q2), 0), fill(ALL(r), 0);\n      repi(j, k) repi(i,\
    \ n)\n      {\n        p2[j * (2 * n) + i] = p[j * n + i];\n        q2[j * (2\
    \ * n) + i] = q[j * n + i];\n        r[j * (2 * n) + i] = i % 2 == 0 ? q[j * n\
    \ + i] : -q[j * n + i];\n      }\n      p2 *= r, q2 *= r;\n      repi(j, k) repi(i,\
    ...
    \ (4 * n) + (2 * i)];\n      }\n    }\n    return (p.rev().pre(m) * (F{1} + (q.rev()\
    \ << 1)).inv(m)).pre(m);\n  }\n}\n#line 7 \"math/fps/compositional_inverse.hpp\"\
    \n\n/**\n * @brief \u9006\u95A2\u6570\n * @docs docs/math/fps/compositional_inverse.md\n\
    \ */\n\n// \u9577\u3055 n \u307E\u3067\ntemplate <class mint>\nFormalPowerSeries<mint>\
    \ compositional_inv\n(\n  const FormalPowerSeries<mint> &f,\n  int n = -1\n)\n\
    {\n  using F = FormalPowerSeries<mint>;\n  assert(f.get(0) == 0 && f.get(1) !=\
    \ 0);\n  if (n < 0)\n    n = f.size();\n  if (n == 0)\n    return {};\n  if (n\
    \ == 1)\n    return {0};\n  const mint c = f[1], ic = c.inv();\n  F h = power_projection(n\
    \ - 1, n, f * ic, F{1});\n  dump(h);\n  repi(i, 1, n) h[i] *= (n - 1) * Binomial<mint>::inv(i);\n\
    \  reverse(ALL(h)), h.pop_back();\n  dump(h);\n  assert(h.get(0) == 1);\n  F g\
    \ = h.pow(-mint(n - 1).inv().val());\n  g.insert(g.begin(), 0);\n  mint pw = 1;\n\
    \  repi(i, 1, n) pw = pw * ic, g[i] *= pw;\n  return g;\n}\n#line 18 \"verify/yosupo/fps_compositional_inverse.test.cpp\"\
    ...
    n\\n\", \"33\");\n    local(while (true))\n    {\n      dump(\"T\");\n      IN(uint,\
    \ T);\n      while (T--)\n      {\n        dump(\"new testcase\");\n        main2();\n\
    \      }\n    }\n    #endif\n  }\n};\n#line 37 \"verify/yosupo/fps_compositional_inverse.test.cpp\"\
  ...
  verifiedWith: []
documentation_of: verify/yosupo/fps_compositional_inverse.test.cpp
layout: document
redirect_from:
- /verify/verify/yosupo/fps_compositional_inverse.test.cpp
- /verify/verify/yosupo/fps_compositional_inverse.test.cpp.html
