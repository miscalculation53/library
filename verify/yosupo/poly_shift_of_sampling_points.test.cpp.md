...
data:
  ...
  bundledCode: "#line 1 \"verify/yosupo/poly_shift_of_sampling_points.test.cpp\"\n\
    ...
    \u306E f(x) \u306B\u3064\u3044\u3066 f(c) \u3092\u6C42\u3081\u308B\n// O(n)\n\
    template <class mint>\nmint shift_of_sampling_points_one(const vc<mint> &ys, const\
    \ mint &c)\n{\n  const int n = ys.size();\n  assert(n > 0);\n  vc<mint> prodr(n\
    \ + 1);\n  prodr[n] = 1;\n  repi(i, n - 1, -1, -1) prodr[i] = prodr[i + 1] * (c\
    \ - i);\n  mint res = 0, prodl = 1;\n  repi(i, n)\n  {\n    res += prodl * prodr[i\
    \ + 1] * ys[i] * ipow(-1, n - 1 - i) * Binomial<mint>::finv(i) * Binomial<mint>::finv(n\
    \ - 1 - i);\n    prodl *= c - i;\n  }\n  return res;\n}\n\n// f(i) = ys[i] (0\
    \ <= i < n) \u3067\u5B9A\u307E\u308B n \u6B21\u672A\u6E80\u306E f(x) \u306B\u3064\
    \u3044\u3066 f(c), ..., f(c + m - 1) \u3092\u6C42\u3081\u308B\n// O((n+m) log\
    \ (n+m))\ntemplate <class mint>\nvc<mint> shift_of_sampling_points_many(const\
    \ vc<mint> &ys, const mint &c, int m)\n{\n  using F = FormalPowerSeries<mint>;\n\
    \  const int n = ys.size();\n  assert(n > 0);\n  assert(m >= 0);\n  F p(n);\n\
    \  repi(i, n) p[i] = i % 2 == 0 ? 1 : -1;\n  F a = (F(ys).egf() * p.egf()).pre(n);\n\
    \  F q(n);\n  q[0] = 1;\n  repi(i, 1, n) q[i] = q[i - 1] * (c - i + 1);\n  F b\
    \ = (a.ogf() * q.egf().rev()) >> (n - 1);\n  F r(m, 1);\n  return (b.egf() * r.egf()).pre(m).ogf();\n\
    }\n#line 21 \"verify/yosupo/poly_shift_of_sampling_points.test.cpp\"\n\nvoid init()\
    \ {}\n\nvoid main2()\n{\n  LL(N, M);\n  IN(mint, c);\n  VEC(mint, N, y);\n  auto\
    \ z = shift_of_sampling_points_many(y, c, M);\n  PRINT(z);\n}\n\nvoid test()\n\
