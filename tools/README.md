...
4. 元コードへのリンク以外のコメントと無効なプリプロセッサ分岐を除き、連続する
   空行を1行にまとめる。
5. 最終出力から `#line` を除く。
...
2. インデントと行内の空白を保ったまま元コードへのリンク以外のコメントを除き、
   連続する空行を1行にまとめて構文を確認する。
3. 65,536 B 未満なら、その候補をアトミックに書き出す。
4. 無効な分岐の除去と無名名前空間化を行う。
5. GCCのテンプレート解析を行い、実体化されなかった関数やメソッドの本体と不要な宣言を削除する。
6. コンパイラに加えて `cppcheck` も使い、既定では最大3周解析する。
7. 最後にGCCのIPA解析を行い、外部から到達しない関数群を削除する。
