...
data:
  ...
  attributes:
    _deprecated_at_docs: docs/ds/uf/undo.md
    document_title: "undo \u53EF\u80FD\u306A vector \u306A\u3069"
    links: []
  bundledCode: "#line 2 \"ds/undo.hpp\"\n\n#line 2 \"template/template_all_but_modint.hpp\"\
    ...
    \ void reset() { his.resize(1), snap_time = 1; }\n  inline void snapshot() { snap_time\
    \ = his.size(); }\n  inline void rollback() { undo(SZ(his) - snap_time); }\n};\n\
    \ntemplate <class T>\nstruct VectorUndo\n{\nprivate:\n  vc<T> vec;\n  vc<pair<int,\
    \ T>> his;\n  int snap_time;\n\npublic:\n  VectorUndo() : snap_time(0) {}\n  VectorUndo(int\
    \ n) : vec(n), snap_time(0) {}\n  template <class Iter>\n  VectorUndo(const Iter\
    \ &bg, const Iter &ed) : vec(bg, ed), snap_time(0) {}\n  VectorUndo(const vc<T>\
    \ &vec) : vec(vec), snap_time(0) {}\n\n  T get(int i) const\n  {\n    assert(0\
  ...
  code: "#pragma once\n\n#include \"../template/template_all_but_modint.hpp\"\n\n\
    ...
    \ times = 1) { his.pop_back(); }\n  inline void reset() { his.resize(1), snap_time\
    \ = 1; }\n  inline void snapshot() { snap_time = his.size(); }\n  inline void\
    \ rollback() { undo(SZ(his) - snap_time); }\n};\n\ntemplate <class T>\nstruct\
    \ VectorUndo\n{\nprivate:\n  vc<T> vec;\n  vc<pair<int, T>> his;\n  int snap_time;\n\
    \npublic:\n  VectorUndo() : snap_time(0) {}\n  VectorUndo(int n) : vec(n), snap_time(0)\
    \ {}\n  template <class Iter>\n  VectorUndo(const Iter &bg, const Iter &ed) :\
    \ vec(bg, ed), snap_time(0) {}\n  VectorUndo(const vc<T> &vec) : vec(vec), snap_time(0)\
    \ {}\n\n  T get(int i) const\n  {\n    assert(0 <= i && i < SZ(vec));\n    return\
    \ vec[i];\n  }\n  void set(int i, T val)\n  {\n    assert(0 <= i && i < SZ(vec));\n\
    \    his.eb(i, vec[i]);\n    vec[i] = val;\n  }\n\n  void undo(int times = 1)\n\
    \  {\n    repi(_, times)\n    {\n      assert(!his.empty());\n      cauto & [\
...
documentation_of: ds/undo.hpp
layout: document
redirect_from:
- /library/ds/undo.hpp
- /library/ds/undo.hpp.html
