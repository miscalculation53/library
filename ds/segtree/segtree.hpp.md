...
- コンストラクタにモノイドを構造体として渡す（重要）
- コンストラクタにイテレータを渡すこともできる（必須ではないが便利）
  - たとえばモノイドにするときに列の要素に加えて長さなどを持っておかなければならないという場面がある。このような場合でも、列の要素を `S` のコンストラクタに渡せるように実装すれば、もとの vector をイテレータとしてセグメント木のコンストラクタに渡すことができる。