...
data:
  ...
  bundledCode: "#line 2 \"math/fps/interpolation.hpp\"\n\n#line 2 \"template/template_all_but_modint.hpp\"\
    ...
    \ dc, int l, int r) -> F\n  {\n    if (r - l == 0)\n      return {1};\n    if\
    \ (r - l == 1)\n      return fs[l];\n    const int m = (l + r) / 2;\n    F f =\
    \ convolution(dc(dc, l, m), dc(dc, m, r));\n    if (d < 0 || d >= (int)f.size())\n\
    \      return f;\n    return F(f.begin(), f.begin() + d);\n  };\n  return dc(dc,\
    \ 0, fs.size());\n}\n#line 2 \"math/fps/multipoint_evaluation.hpp\"\n\n#line 4\
    \ \"math/fps/multipoint_evaluation.hpp\"\n\n#line 6 \"math/fps/multipoint_evaluation.hpp\"\
    \n\n/**\n * @brief \u591A\u9805\u5F0F\u306E\u591A\u70B9\u8A55\u4FA1\n * @docs\
    \ docs/math/fps/multipoint_evaluation.md\n */\n\ntemplate <class mint>\nvc<mint>\
    \ multipoint_evaluation(const FormalPowerSeries<mint> &f, const vc<mint> &xs)\n\
    {\n  using F = FormalPowerSeries<mint>;\n  const int m0 = xs.size(), m = bit_ceil(m0),\
    \ h = min(6, (int)bit_width(m0) - 1);\n  vc<F> node(2 * m, {1});\n  repi(i, m0)\
    \ node[m + i] = {-xs[i], 1};\n  repi(i, m - 1, 0, -1) node[i] = convolution(node[2\
    \ * i], node[2 * i + 1]);\n  node[1] = f % node[1];\n  repi(i, 2, m >> (h - 1))\
    \ node[i] = node[i / 2] % node[i];\n  vc<mint> res(m0);\n  repi(i, m0) res[i]\
    \ = node[(m + i) >> h].eval(xs[i]);\n  return res;\n}\n#line 2 \"math/fps/rational_sum.hpp\"\
    ...
    \ z = bit_ceil(max({k + n - 1, l + m - 1, l + n - 1}));\n  // (ps + qr) / qs\n\
    \  if (!ntt_ok<mint>(z) || min({p_.cnt_nz(), q_.cnt_nz(), r_.cnt_nz(), s_.cnt_nz()})\
    \ <= 60)\n    return {p_ * s_ + q_ * r_, q_ * s_};\n  // NTT \u3092\u4F7F\u3044\
    \u56DE\u3059\n  F p = p_, q = q_, r = r_, s = s_;\n  p.resize(pz), q.resize(z),\
    \ r.resize(rz), s.resize(z);\n  ntt(p), ntt(q), ntt(r), ntt(s);\n  repi(i, pz)\
    \ p[i] *= s[i];\n  repi(i, rz) r[i] *= q[i];\n  repi(i, z) q[i] *= s[i];\n  intt(p),\
