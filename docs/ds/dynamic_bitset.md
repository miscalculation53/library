...
`==`, `!=` は長さと各ビットの一致を判定する。長さが異なる場合、`==` は `false` を返す。

大小比較では、第0ビットを最下位ビットとして、
$v(a)=\sum_{i=0}^{n-1} a[i]2^i$ の大小を比較する。
例えば `DynamicBitset("0011") < DynamicBitset("0100")` は `true` となる。
...
- $O(1+n/w)$。大小比較は上位ワードから調べ、異なるワードが見つかると終了する
