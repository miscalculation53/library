...
となる。可換性により、条件を満たす更新について $(w,w^{[b]},w^{[a]},w^{[ab]})$ の $4$ 成分をそれぞれ集約すればよい。4成分に `G::op` を成分ごとに適用する直積モノイドを定義し、既存の `FenwickTree` の一つの要素として載せる。座標圧縮・ソート・Fenwick Tree 上の走査はいずれも一度で済む。
